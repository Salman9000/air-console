...
export default function Money({ addMoney, bucketRect }) {
  ...
  const tick = () => {
    ...
    money.forEach(item => {
      ...
      if (item.spawnTime + (item.duration*1000) > Date.now() || item.isDragging) {
        newMoney.push(item);
      }
    });
    if (money.length < 10) {
        ...
        <div key={item.id} className={item.id} >
          <Draggable
            ...
            onDrag={(e) => {
              ...
              console.log(e)
              if (x > bucketRect.left && x < bucketRect.right && y > bucketRect.top && y < bucketRect.bottom) {
                // kill this item
                const moneyCopy = [...money];
          ...
          >
            <div ref={(element) => (moneyRef.current[i] = element)}>
              <div style={{
                ...
                <div className={` transform  h-8 ${item.negative > 0.5 ? "-" : ""
                      }rotate-${item.rot
